...
const HearingTest = ({route}) => {
  const {babyDetails, docId} = route.params;

  const [hearingScreens, setHearingScreens] = useState([
    {
      id: 1,
      date1: '',
      date2: '',
  ...
  useEffect(() => {
    if (!isEmpty(babyDetails?.hearingResults)) {
      setHearingScreens(
  ...
  const onResetPress = () => {
    console.log('2====>', babyDetails.hearingResults);
    if (!isEmpty(babyDetails?.hearingResults)) {
      setHearingScreens(
        babyDetails.hearingResults.map(item => ({
          ...item,
  ...
  const onSavePress = () => {
    setIsLoading(true);
    const results = hearingScreens.map(item => {
      return {
        date1: item.date1,
        date2: item.date2,
        id: item.id,
    ...
    firebase
      ...
      .then(() => {
        ...
        Alert.alert(
          '',
          'Hearing Screen Results Added Successfully',
  ...
  const renderHearingScreen = hearingScreen => {
    const {id, date1, date2, left, right, openDatePicker1, openDatePicker2} =
      hearingScreen;

    const isLeftDisabled = !isEmpty(left) || !isEmpty(date1);
    const isRightDisabled = !isEmpty(right) || !isEmpty(date1);

    return (
      <View key={id} style={{marginTop: hp(3)}}>
        <Text style={style.hearingScreenTextStyle}>Hearing Screen {id}</Text>

        <Button
          buttonText={date1 ? date1 : `Add Hearing Screen ${id} date`}
          onPress={() =>
            setHearingScreens(prevState => {
              const updatedHearingScreens = [...prevState];
              updatedHearingScreens[id - 1].openDatePicker1 = true;
              return updatedHearingScreens;
            })
          }
            ...
            <View style={{flexDirection: 'row'}}>
              <CheckBox
                ...
                onPress={() =>
                  setHearingScreens(prevState => {
                    const updatedHearingScreens = [...prevState];
                    updatedHearingScreens[id - 1].left = 'Passed';
                    return updatedHearingScreens;
                  })
                }
              />
            ...
            <View style={style.checkboxSecTextStyle}>
              <CheckBox
                ...
                onPress={() =>
                  setHearingScreens(prevState => {
                    const updatedHearingScreens = [...prevState];
                    updatedHearingScreens[id - 1].left = 'Refer';
                    return updatedHearingScreens;
                  })
                }
              />
            ...
            <View style={{flexDirection: 'row'}}>
              <CheckBox
                ...
                onPress={() =>
                  setHearingScreens(prevState => {
                    const updatedHearingScreens = [...prevState];
                    updatedHearingScreens[id - 1].right = 'Passed';
                    return updatedHearingScreens;
                  })
                }
              />
            ...
            <View style={style.checkboxSecTextStyle}>
              <CheckBox
                ...
                onPress={() =>
                  setHearingScreens(prevState => {
                    const updatedHearingScreens = [...prevState];
                    updatedHearingScreens[id - 1].right = 'Refer';
                    return updatedHearingScreens;
                  })
                }
              />
        ...
        <Button
          ...
          onPress={() =>
            setHearingScreens(prevState => {
              const updatedHearingScreens = [...prevState];
              updatedHearingScreens[id - 1].openDatePicker2 = true;
              return updatedHearingScreens;
            })
          }
        ...
        <DateTimePicker
          ...
          onConfirm={date =>
            handleDatePickerConfirm(
              ...
              setHearingScreens(prevState => {
                const updatedHearingScreens = [...prevState];
                updatedHearingScreens[id - 1].date1 =
                  moment(date).format('DD/MM/YYYY');
                updatedHearingScreens[id - 1].openDatePicker1 = false;
                return updatedHearingScreens;
              }),
            )
          }
        ...
        <DateTimePicker
          ...
          onConfirm={date =>
            handleDatePickerConfirm(
              ...
              setHearingScreens(prevState => {
                const updatedHearingScreens = [...prevState];
                updatedHearingScreens[id - 1].date2 =
                  moment(date).format('DD/MM/YYYY');
                updatedHearingScreens[id - 1].openDatePicker2 = false;
                return updatedHearingScreens;
              }),
            )
          }
  ...
  return (
    <View style={style.containerStyle}>
      ...
      <Text style={style.titleStyle}>Hearing Test</Text>
      ...
      <ScrollView contentContainerStyle={style.contentContainerStyle}>
        {hearingScreens.map(hearingScreen =>
          renderHearingScreen(hearingScreen),
        )}
      </ScrollView>
